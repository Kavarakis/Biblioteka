...
namespace BibliotekaZadaca2
{
    public abstract class Knjiga
    {
        ...
        protected int IdKnjiga;
        protected static int brojKopija{get;set;}
        protected string naslov { get; set; }
        protected Autori autori;
        protected string godinaIzdanja { get; set; }
        protected string nazivIzdavaca { get; set; }
        protected string zanr { get; set; }
        protected string isbn;
        protected DateTime DatumIznamljivanja { get; set; }
        protected bool iznajmljena { get; set; } = false;
        public Knjiga(string Naslov, string autor, string godina, string izdavac, string zanr, string ISBN)
        {
            brojac++;
            IdKnjiga=brojac;
            //this.naslov = naslov;
            Authors= new Autori (autor);
            CheckISBN = ISBN;
        ...
        public string CheckISBN
        {
            ...
            set
            {
                if (TryValidateISBN(value)) this.isbn = value;
                else throw new ArgumentException("Pogresan ISBN!");
            }
        }
        public static bool TryValidateISBN(string isbn)
        {
            bool result = false;
       ...
       public virtual bool StanjeIznajmljivanja()
        {
            return iznajmljena;
        }
        public virtual void SetujIznamljivanje(bool temp)
        {
            DatumIznamljivanja = DateTime.Today;
            iznajmljena = temp;
        }
